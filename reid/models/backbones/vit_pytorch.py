...
class TransReID(nn.Module):
    ...
    def forward(self, x, **kwargs):
        B = x.shape[0]
        x = self.patch_embed(x)  # get patch embeddings
        ...
        for blk in self.blocks:
            x = blk(x)
        x = self.norm(x)
        # get the cls feature for further prediction
        return x[:, 0]
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                if 'distilled' in model_path:
                    ...
                    # interpolate for pos emb
                    cls_token_pos, img_token_pos = v[:, :1], v[:, 1:]
                    img_token_pos = img_token_pos.unsqueeze(0)
                    feature_dim = self.pos_embed.shape[2]
                    expand_dim = self.pos_embed.shape[1] - 1
                    img_token_pos = F.interpolate(
                        img_token_pos, size=(
                            expand_dim, feature_dim), mode='bilinear'
                    ).squeeze(0)
                    v = torch.cat([cls_token_pos, img_token_pos], 1)
...
def resize_pos_embed(posemb, posemb_new, hight, width):
    ...
    posemb_grid = F.interpolate(
        posemb_grid, size=(hight, width), mode='bilinear')
    posemb_grid = posemb_grid.permute(0, 2, 3, 1).reshape(1, hight * width, -1)
    posemb = torch.cat([posemb_token, posemb_grid], dim=1)
    return posemb


def vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0,
